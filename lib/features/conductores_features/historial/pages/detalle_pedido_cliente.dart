...
class DetallePedidoClientePage extends StatefulWidget {
  ...
  @override
  State<DetallePedidoClientePage> createState() =>
      _DetallePedidoClientePageState();
}

class _DetallePedidoClientePageState extends State<DetallePedidoClientePage> {
  ...
  // ✅ Cliente UID: lo busca en root "uid" y si no, en ubicacion.uid
  String _clienteUidFromData(Map<String, dynamic> data) {
    dynamic v;
    if (data.containsKey('uid')) v = data['uid'];

    final s0 = _asString(v).trim().toLowerCase();
    final isEmptyOrNull = s0.isEmpty || s0 == 'null';

    if (isEmptyOrNull && data['ubicacion'] is Map) {
      final u = Map<String, dynamic>.from(data['ubicacion']);
      v = u['uid'];
    }
  ...
  _TrackStep _stepFromRaw(String raw) {
    ...
    if (s == 'aceptado' || s == 'aceptada') return _TrackStep.aceptado;
    if (s == 'en_curso' ||
        s == 'en curso' ||
        s == 'encurso' ||
        s == 'en_camino' ||
  ...
  _BadgeMeta _badgeFromStep(_TrackStep step) {
    switch (step) {
      case _TrackStep.completado:
        return _BadgeMeta('Completado', Icons.check_circle_outline_rounded,
            Palette.statsSuccess);
      case _TrackStep.cancelado:
        return _BadgeMeta(
            'Cancelado', Icons.cancel_outlined, Palette.statsDanger);
      case _TrackStep.retrasado:
        return _BadgeMeta(
            'Retrasado', Icons.warning_amber_rounded, Palette.statsWarning);
      case _TrackStep.enCurso:
        return _BadgeMeta('En curso', Icons.local_shipping_outlined,
            Palette.statsWarning);
      case _TrackStep.aceptado:
        return _BadgeMeta(
            'Aceptado', Icons.verified_outlined, Palette.statsSuccess);
      case _TrackStep.pendiente:
        return _BadgeMeta(
            'Pendiente', Icons.timelapse_rounded, Palette.statsWarning);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // ✅ AppBar sencillo (lee código del doc si widget.pedidoCode viene vacío)
            Padding(
                  ...
                  Expanded(
                    child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                      ...
                      builder: (context, s) {
                        final d = s.data?.data() ?? {};
                        final codigoDoc =
                            (d['codigo'] ?? d['pedidoCode'] ?? '').toString().trim();

                        final code = widget.pedidoCode.trim().isNotEmpty
                            ? widget.pedidoCode.trim()
                            : (codigoDoc.isNotEmpty ? codigoDoc : '—');
              ...
              child: !_isLoggedIn()
                  ...
                  : StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                      ...
                      builder: (context, snap) {
                        ...
                        // ✅ Cliente UID y doc usuario
                        final clienteUid = _clienteUidFromData(data);
                        final userDoc = clienteUid.isEmpty
                            ? null
                            : _fire.collection('usuarios').doc(clienteUid);
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      children: [
                                        ...
                                        // ✅ CLIENTE (por uid) - NOMBRE y CORREO separados
                                        if (userDoc == null) ...[
                                          _ResumenRow(
                                            label: 'Cliente',
                                            value: clienteUid.isEmpty
                                                ? '—'
                                                : clienteUid,
                                            valueColor: clienteUid.isEmpty
                                                ? Palette.statsWarning
                                                : purple,
                                          ),
                                          ...
                                          _ResumenRow(
                                            label: 'Correo',
                                            value: '—',
                                            valueColor: Palette.statsWarning,
                                          ),
                                        ] else
                                          StreamBuilder<
                                              DocumentSnapshot<
                                            ...
                                            builder: (context, uSnap) {
                                              ...
                                              final nombreShow = nombre.isNotEmpty
                                                  ? nombre
                                                  : (clienteUid.isNotEmpty
                                                      ? clienteUid
                                                      : '—');

                                              final correoShow = correo.isNotEmpty
                                                  ? correo
                                                  : '—';
